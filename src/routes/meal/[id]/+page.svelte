<script lang="ts">
    ...
    let comments = $state<{id: string, text: string, timestamp: number}[]>([]);
    ...
    $effect(() => {
        if (schoolCode && date) {
            ...
            const storedVotes = localStorage.getItem(`votes-${mealId}`);
            if (storedVotes) {
                votes = parseInt(storedVotes, 10);
            }

            const storedComments = localStorage.getItem(`comments-${mealId}`);
            if (storedComments) {
                comments = JSON.parse(storedComments);
            }

            const storedHasVoted = localStorage.getItem(`hasVoted-${mealId}`);
            if (storedHasVoted) {
    ...
    function handleVote() {
        ...
        votes++;
        hasVoted = true;
        localStorage.setItem(`votes-${mealId}`, votes.toString());
        localStorage.setItem(`hasVoted-${mealId}`, 'true');
    }
    ...
    function handleAddComment() {
        ...
        localStorage.setItem(`comments-${mealId}`, JSON.stringify(comments));
        newComment = "";
    }
                        ...
                        {:else}
                            {#each comments.slice().reverse() as comment}
                                <div class="bg-gray-50 rounded-lg p-4">
                                    ...
                                    <div class="text-xs text-gray-400">{formatTimestamp(comment.timestamp)}</div>
                                </div>
                            {/each}
                        {/if}
                    </div>
