	...
	<div class="mx-auto max-w-3xl">
		...
		{:else if error}
			<div class="py-20 text-center">
				<div class="mb-4 font-medium text-red-500">{error}</div>
		...
		{:else if meal}
			<div class="space-y-6">
				<div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
					<div
						class="h-2 bg-linear-to-r {mealTypeColors[meal.type] || 'from-gray-400 to-gray-500'}"
					></div>
					<div class="p-6">
						...
						<div class="mb-6 text-sm text-gray-500">{meal.calories}</div>
						...
						<div class="flex items-center gap-4 border-t border-gray-100 pt-4">
							<Button
								...
								disabled={hasVoted}
							>
								<ThumbsUp class="h-4 w-4 {hasVoted ? 'fill-current' : ''}" />
		...
		{:else}
			<div class="py-20 text-center">
				<div class="mb-4 text-gray-500">급식 정보를 찾을 수 없습니다.</div>
				<Button onclick={() => history.back()}>이전 페이지로 돌아가기</Button>
			</div>
		{/if}
	</div>
