<script lang="ts">
	...
	$effect(() => {
		if (selectedSchool && selectedDate && selectedOfficeCode) {
			...
			fetch(
				`/api/meal/${selectedSchool}?from=${dateStr}&to=${dateStr}&officeCode=${selectedOfficeCode}`
			)
				.then(async (res) => {
...
<div class="min-h-screen bg-gray-50/50 p-6 md:p-12">
	<div class="mx-auto max-w-5xl">
		...
		<div class="grid grid-cols-1 items-start gap-8 md:grid-cols-[350px_1fr]">
			<div class="flex flex-col gap-6 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
				<div class="space-y-4">
					<div class="space-y-2">
							...
							<Select.Root type="single" bind:value={selectedSchool}>
								...
								<Select.Content>
									{#each schools as school}
										<Select.Item value={school.schoolCode} label={school.schoolName}
											>{school.schoolName}</Select.Item
										>
									{/each}
								</Select.Content>
							</Select.Root>
				...
				<div class="space-y-2 border-t border-gray-100 pt-4">
					<span class="block text-sm font-medium text-gray-700">날짜 선택</span>
					<div class="flex justify-center">
						<Calendar
							type="single"
							bind:value={selectedDate}
							class="rounded-xl border bg-white shadow-xs"
							locale="ko-KR"
						/>
					</div>
				</div>
			...
			<div class="min-h-[500px] rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
				{#if selectedSchool}
					{#if meals.length > 0}
						<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
							{#each meals as meal}
								<a
									href="/meal/{encodeURIComponent(meal.id)}"
									class="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-xl border border-gray-200 bg-white p-6 shadow-xs transition-all hover:border-blue-200 hover:shadow-md"
								>
									<div
										class="absolute top-0 left-0 h-1 w-full bg-linear-to-r
                                        {meal.type === '조식'
									...
									<div class="flex-1 space-y-2 text-sm leading-relaxed text-gray-700">
									...
									<div
										class="mt-4 flex items-center gap-4 border-t border-gray-100 pt-3 text-sm text-gray-500"
									>
					...
					{:else}
						<div
							class="flex h-full flex-col items-center justify-center py-20 text-center text-gray-500"
						>
							<div class="mb-4 rounded-full bg-gray-100 p-4">
								<CircleAlert class="h-8 w-8 text-gray-400" />
							</div>
							<p class="mb-1 text-lg font-medium text-gray-900">
								선택한 기간의 급식 정보가 없습니다
							</p>
				...
				{:else}
					<div
						class="flex h-full flex-col items-center justify-center py-20 text-center text-gray-500"
					>
						<div class="mb-4 rounded-full bg-blue-50 p-4 text-blue-500">
							<Search class="h-8 w-8" />
						</div>
						<p class="mb-1 text-lg font-medium text-gray-900">학교를 선택해주세요</p>
